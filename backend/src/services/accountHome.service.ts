...
const findAll = async (): Promise<ServiceResponse<MatchesModel[]>> => {
  const allUsers = await MatchesModel.findAll({ include: [
    { model: TeamsModel,
      as: 'homeTeam',
      attributes: ['teamName'],
    },
  ...
  return (!allUsers)
    ? { type: 'NADA_PRESENTE', data: { message: 'Nada aqui' } }
    : { type: null, data: allUsers };
};

const insertCashbackToTransaction = async (
  id: number,
  ...
  return { type: null, data: { message: 'Matche Updated' } };
};

const verifyExistTransaction = async (id1: number, id2: number):
Promise<ServiceResponse<{ message: string }>> => {
...
const insertTransaction = async (
  ...
  const { type, data } = await verifyExistTransaction(homeTeamId, awayTeamId);
